...
double const fm_gain = 2.5; // FM emulators are internally quieter to avoid 16-bit overflow
...
void set_voice( struct Vgm_Emu* this, int i, struct Blip_Buffer* c, struct Blip_Buffer* l, struct Blip_Buffer* r )
{
	if ( i < osc_count ) {
		int j;
		for ( j = osc_count; --j >= 0; )
			Apu_set_output( &this->psg, j, c, l, r );
	}
}
...
blargg_err_t setup_fm( struct Vgm_Emu* this )
{
	...
	if ( uses_fm( this ) )
	{
		...
		Apu_volume( &this->psg, 0.195 * fm_gain * this->gain );
	}
	else
	{
		Apu_volume( &this->psg, this->gain );
	}
...
blargg_err_t run_clocks( struct Vgm_Emu* this, blip_time_t* time_io, int msec )
{
	*time_io = run( this, msec * this->vgm_rate / 1000 );
	Apu_end_frame( &this->psg, *time_io );
	return 0;
}
...
blip_time_t run( struct Vgm_Emu* this, vgm_time_t end_time )
{
	...
	while ( vgm_time < end_time && pos < this->file_end )
	{
		...
		case cmd_gg_stereo:
			Apu_write_ggstereo( &this->psg, to_psg_time( this, vgm_time ), *pos++ );
			break;
		
		case cmd_psg:
			Apu_write_data( &this->psg, to_psg_time( this, vgm_time ), *pos++ );
			break;
...
int play_frame( struct Vgm_Emu* this, blip_time_t blip_time, int sample_count, blip_sample_t out [] )
{
	...
	Apu_end_frame( &this->psg, blip_time );
...
void Sound_mute_voices( struct Vgm_Emu* this, int mask )
{
	...
	{
		for ( i = osc_count; --i >= 0; )
			Apu_set_output( &this->psg, i, ( mask & 0x80 ) ? 0 : &this->stereo_buf.bufs [0], NULL, NULL );
		if ( Ym2612_enabled( &this->ym2612 ) )
		{
...
blargg_err_t Vgm_start_track( struct Vgm_Emu* this )
{
	...
	Apu_reset( &this->psg, get_le16( header( this )->noise_feedback ), header( this )->noise_width );
