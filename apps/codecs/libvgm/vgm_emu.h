...
static inline void Vgm_disable_oversampling( struct Vgm_Emu* this_, bool disable ) { this_->disable_oversampling = disable; }

// Header for currently loaded file
static inline struct header_t *header( struct Vgm_Emu* this_ ) { return (struct header_t*) this_->file_begin; }
...
// is called.
static inline bool uses_fm( struct Vgm_Emu* this_ ) { return Ym2612_enabled( &this_->ym2612 ) || Ym2413_enabled( &this_->ym2413 ); }

// Set output sample rate. Must be called only once before loading file.
blargg_err_t Vgm_set_sample_rate( struct Vgm_Emu* this_, long sample_rate );
...
blargg_err_t Vgm_play( struct Vgm_Emu* this_, long count, sample_t* buf ); ICODE_ATTR
...
// Skip n samples
blargg_err_t Track_skip( struct Vgm_Emu* this_, long n );
...
static inline long Track_get_length( struct Vgm_Emu* this )
{
	...
	if ( length <= 0 )
	{
		length = this->info.intro_length + this->info.loop_length; // intro + 1 loop
		if ( length <= 0 )
...
static inline void Sound_set_gain( struct Vgm_Emu* this_, double g )
{
	assert( !this_->sample_rate ); // you must set gain before setting sample rate
	this_->gain = g;
}
