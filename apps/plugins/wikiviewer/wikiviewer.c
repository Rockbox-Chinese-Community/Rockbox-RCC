...
#define WIKIVIEWER_END_RECORD (BUTTON_SELECT|BUTTON_REPEAT)
...
#define WIKIVIEWER_SELECT BUTTON_SELECT
#define WIKIVIEWER_END_RECORD (BUTTON_REC|BUTTON_MODE)
...
static int lastlinkcount=0;        /* set by render() to the number of links found */
...
/*
  ...
  1) load the article given by res_lo, res_hi
  2) linestate=0
  3) while linestate.renderoff<histstate.curoff && !eof, advance
*/
...
static int record_flag=0;   /*set to 1 when recording a line to an external file*/
static int last_line_i=0;   /*set to linespace when recording a line to an external file*/
        ...
        for (x=1,i=0,linedone=0;!linedone && i<LINEBUFLEN && cur.renderlen>0;) {
            ...
            switch (articlebuf[cur.renderoff]) {
                ...
                default:
                    ...
                    /* but multiple other whitespace will be ignored */
...
loadnewarticle:
    ...
    if (target) {
        ...
        while (navcur!=navnext) {
            ...
            /* navnext will be pointing at the start of the line with the
               target in it

            * Attempt match, this is a bit tricky (and it is probably equally tricky to get
            * an anchor pointing to another document in wikicode in the first place).
            */
    ...
    while (!wrquit) {
	    ...
	    case WIKIVIEWER_BACK2_REPEAT:
                ...
                case MODE_NORMAL:
#ifdef WIKIVIEWER_BACK2
                    if (button==BUTTON_UP) {          /*scroll up page*/
