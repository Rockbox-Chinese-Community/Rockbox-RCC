...
generates output_prefix.wwr, output_prefix.wwt, output_prefixN.wwa where
N is 0-? (each file ~ 1 GB)

This is a rewrite from original ruby converter (2007 by freqmod)
*/
...
/*
   write a unicode value to a character buffer as UTF-8
   increments *offset by numer of bytes written to buf
   by hcs, based on the wikipedia article on UTF-8
*/
...
/*
   ...
   inbuf: input buffer
   inlength: total length of input buffer (few parsers will consume the entire input),
             decremented by number of bytes consumed
*/
...
PARSEHEAD(linkname)
{
    if (inbuf[0]=='|') {
        CONSUME(1);
        /* take back anything we've written so far, only the last is display text */
        outbuf=*outbufptr;
...
static inline void doparse_text(char ** outbufptr, const char ** inbufptr, int * const inlenptr) {
    ...
    case '&':
        ...
        if (!memcmp(inbuf+1,"amp;",4)) {
            /*
              might contain another entitized entity
              (&ndash; appears in the dump as &amp;ndash;)
            */
...
int main(int argc,char * argv[])
{
    ...
    while (fgets(linebuffer,8191,xmldump)!=0)
    {
        ...
        if(matches_open>=2)
        {
            ...
            {
                if((matches_open=pcre_exec(regex_betweentags, NULL, linebuffer, strlen(linebuffer), 0, PCRE_NOTEMPTY, ovector_open, 30))<1) printf("title empty\n");
                else
                {
